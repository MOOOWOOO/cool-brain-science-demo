...
class User(db.Model):
    ...
    historys = db.relationship('UserSurveyHistory', backref="user", lazy=False)
...
class UserSurveyHistory(db.Model):
    __tablename__ = 'user_servey_history'

    id = db.Column(db.Integer, primary_key=True)
    viewed_at = db.Column(db.DateTime, default=datetime.utcnow)
    score = db.Column(db.Integer, default=0)
    choice_list = db.Column(db.Text)
...
class Survey(db.Model):
    ...
    historys = db.relationship('UserSurveyHistory', backref="survey", lazy=False)
